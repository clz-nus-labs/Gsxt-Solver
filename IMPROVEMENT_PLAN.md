...
1. Annotate the 50-image suite and implement metrics.
2. Save top-k character and icon candidates in every result.
3. Implement global assignment and strict explicit-target output.
4. Add character beam search with lexicon/language-model scoring.
5. Train prompt-to-body icon embeddings.
6. Retrain the detector with hard negatives and region-role labels.
7. Replace threshold-only task selection with multi-hypothesis scoring.
8. Export static models and simplify installation.
