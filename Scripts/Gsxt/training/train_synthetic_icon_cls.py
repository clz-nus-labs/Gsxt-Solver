...
class IconDataset(Dataset):
    def __init__(self, list_path: Path, label_to_id: dict[str, int], image_size: int, train: bool) -> None:
    ...
    def __getitem__(self, index: int) -> tuple[np.ndarray, np.ndarray]:
        ...
        if self.train and random.random() < 0.55:
...
def main() -> None:
    ...
    train_ds = IconDataset(dataset_dir / "train.txt", label_to_id, args.image_size, train=True)
    val_ds = IconDataset(dataset_dir / "val.txt", label_to_id, args.image_size, train=False)
    ...
    for epoch in range(state.epoch + 1, args.epochs + 1):
        ...
        for images, labels_tensor in iterator:
            logits = model(images)
            loss = F.cross_entropy(logits, labels_tensor, weight=weight_tensor)
            loss.backward()
            optimizer.step()
